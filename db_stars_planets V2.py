'''   EXOPLANETS V2.0 join Tables for the entire panetary system of a star
...
'''
#CREATES DATABASE FROM A CSV FOR STARS and PLANETS
...
FROM Star AS s, Planet AS p #AS give an alias
WHERE s.kepler_id = p.kepler_id; #join condition

SELECT s.radius AS sun_radius, p.radius AS planet_radius
FROM Star AS s, Planet AS p
WHERE s.kepler_id = p.kepler_id AND (s.radius / p.radius > 1)#select star-planet system where the ratio Sun-to-Earth is greater than ours  
ORDER BY sun_radius DESC;#ordered by star radius

SELECT Star.kepler_id, Planet.koi_name
FROM Star
JOIN Planet USING (kepler_id);#JOIN avoid using equality condition(WHERE s.kepler_id = p.kepler_id;)

SELECT Star.radius, COUNT(Planet.koi_name)#Count the numbers of planets
FROM Star
JOIN Planet USING (kepler_id)             #Join the databases from their kepler_id
WHERE Star.radius > 1                     #In stars with a radius greater than sun radius
GROUP BY Star.radius, Star.kepler_id      #Group by the star radius and the name of the star
HAVING COUNT(Planet.koi_name) > 1         #Only is the star have more than one planet
ORDER BY Star.radius DESC;                #Sorted by the radius of the star

SELECT * FROM Star
WHERE Star.radius > (
  SELECT AVG(radius) FROM Star #Subquery to find out the average 
);

\timing #allows to know the time that take do a query
...
SELECT ROUND(avg(p.t_eq),1), min(s.t_eff), max(s.t_eff)#Calculate avg of planet t_eq, min and max stars t_eff
FROM Star s
JOIN Planet p USING (kepler_id)
WHERE s.t_eff > (SELECT avg(t_eff) FROM Star ); #From stars that have an t_eff greater than the average of the stars t_eff

SELECT p.koi_name, p.radius, s.radius #Find the planets radius which orbit the five largest stars
FROM Star s
JOIN Planet p USING (kepler_id)
WHERE s.radius IN (
  SELECT radius FROM star #Select only the five largest stars
...
############ QUERIES
SELECT S.kepler_id, S.t_eff, S.radius #Return kepler_id, t_eff and star radius
FROM Star S
LEFT OUTER JOIN Planet P USING(kepler_id) #Joining planet database on Star database
WHERE P.koi_name IS NULL                  #Selecting only the stars without planets
ORDER BY S.t_eff DESC;                    #Sorted by t_eff